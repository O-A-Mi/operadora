...
import {conteudo} from "../index.jsx"

export default function ModalInfo({ onClose, isOpen, initialData, setFuncs, setAtualizarDados }) {
  if(!isOpen) return

  //---------------Dados Cliente---------------//
  const [nome, handleNomeChange, nomeRef] = UseInputMask(null, "text", conteudo.nome);
  const [cpf, handleCpfChange, razao_socialRef] =  UseInputMask("999.999.999-99", "number", conteudo.cpf);
  const [genero, handleGeneroChange, cnpjRef] =  UseInputMask(null, "text" ,conteudo.genero );
  const [dataDeNascimento, handleDataDeNascimentoChange, tipoRef] = UseInputMask( "99/99/9999", "number", conteudo.dataDeNascimento);
  const [email, handleEmailChange, emailRef] =  UseInputMask(null , "text" , conteudo.email);
  const [telefone, handleTelefoneChange, telefoneRef] =  UseInputMask("(99) 9999-9999", "number", conteudo.telefone);
  const [celular, handleCelularChange, CelularRef] =  UseInputMask("(99) 99999-9999", "number", conteudo.celular);
      ...
      <div className={styles.modalContent}>
        ...
        <div className={styles.modalBody}>
          <form className={styles.modalForm}>
            <UseInputPadrao label="Nome"  identifier="nome"  onChange={handleNomeChange} value={nome} readOnly  disabled  inputRef={nomeRef} inputStyle={InputStyle}/>

            <UseInputPadrao label="CPF"  identifier="cpf" value={cpf} onChange={handleCpfChange} readOnly  disabled inputRef={razao_socialRef} inputStyle={InputStyle}/>

            <UseInputPadrao label="Genero"  identifier="genero" value={genero} onChange={handleGeneroChange}  inputRef={cnpjRef} readOnly  disabled inputStyle={InputStyle}/>

            <UseInputPadrao label="Data de Nascimento"  identifier="dataDeNascimento"  value={dataDeNascimento} onChange={handleDataDeNascimentoChange} inputRef={tipoRef} readOnly  disabled inputStyle={InputStyle}/>
            ...
            <UseInputPadrao label="Telefone" identifier="telefone" type="tel" value={telefone} onChange={handleTelefoneChange} onBlur={checkValidaTelefone} inputRef={telefoneRef} />

            <UseInputPadrao label="Celular" required identifier="celular" type="tel" value={celular} onChange={handleCelularChange} onBlur={checkValidaCelular} inputRef={CelularRef} />
          </form>
        </div>
