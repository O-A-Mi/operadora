...
const CadastroBeneficiarios = () => {
  ...
  const [cliente, clienteChange, clienteRef] = UseInputMask('999.999.999-99', "both");
  const [contrato, contratoChange, contratoRef] = UseInputMask();
  ...
  const filtrarBeneficiarios = () => {
    try {
      ...
      const dadosFiltrados = tabelaDados.filter((row) => {
        ...
        return (
          (filtro.cliente === "" || (nome.includes(filtro.cliente.toLowerCase()) || cpf.includes(filtro.cliente))) &&
          (filtro.contrato === "" || (row.contrato || "").includes(filtro.contrato)) &&
  ...
  useEffect(() => {
    const contratos = tabelaDados.map((row) => ({
      value: row.contrato,
      label: row.contrato,
    }));
  ...
  const limparFiltro = () => {
    limparFiltros([
      { setter: clienteChange, ref: clienteRef },
      { setter: contratoChange, ref: contratoRef },
          ...
          <section className={styles.filtroTabelaField}>
            <div className={styles.filtroTabelaContent}>
              ...
              <div className={styles.filtroTabelaBody}>
                <UseInputPadrao
                  label="Cliente" identifier="cliente" icon="fas fa-user"
                  value={cliente} onChange={clienteChange} inputRef={clienteRef}
                ...
                />
                <UseInputPadrao
                  label="Contrato" identifier="contrato"
                  value={contrato} onChange={contratoChange} inputRef={contratoRef}
                  type='select' options={contratosOptions}
                  width={isMobile ? 100 : 33.33} gap={isMobile ? 0 : 0.5}
                  onBlur={() => filtrarBeneficiarios()}
                  onKeyPress={(e) => { if (e.key === 'Enter') filtrarBeneficiarios() }}
                ...
                <div className={styles.filtroTabelaBody}>
                  <UseInputPadrao
                    ...
                    width={isMobile ? 100 : 50} gap={isMobile ? 0 : 0.5}
                    onBlur={() => filtrarBeneficiarios()}
                    onKeyPress={(e) => { if (e.key === 'Enter') filtrarBeneficiarios() }}
                  ...
                  <UseInputPadrao
                    ...
                    width={isMobile ? 100 : 50} gap={isMobile ? 0 : 0}
                    onBlur={() => filtrarBeneficiarios()}
                    onKeyPress={(e) => { if (e.key === 'Enter') filtrarBeneficiarios() }}
                  />
                </div>
              ...
              additionalButtons: [
                {
                  ...
                  onClick: () => navigate(`/${jsonRoute.AreaOperadora}/${jsonRoute.Operacional}/${jsonRoute.CadastroBeneficiarios}/${jsonRoute.CadastroBeneficiarioNovo}`)
                }
              ]
            }}
          />
