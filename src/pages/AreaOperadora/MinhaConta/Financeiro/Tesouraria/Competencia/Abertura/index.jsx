...
import ModalMotivoAbertura from './ModalMotivoAbertura';
...
const Abertura = () => {
  ...
  const [showModalMotivoAbertura, setShowModalMotivoAbertura] = useState(false);
  ...
  const handleResize = useCallback(() => {
    handleResizeTabela('abertura-competencia', 'abertura-competencia-container');
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      ...
      const dados = dadosFinanceiro.tesouraria.competencia.abertura.map(item => ({
        ...item,
  ...
  const handleEfetuarAbertura = useCallback(() => {
    ...
    dialogMessage(
      "Tem certeza que deseja efetuar a abertura deste registro?",
      "info",
      {
        buttonsText: {
          confirm: "Sim",
      ...
      (result) => {
        if (result) {
          setShowModalMotivoAbertura(true);
        }
      }
    );
  }, [selecionados]);

  const handleCloseModalAbertura = useCallback(() => {
    setShowModalMotivoAbertura(false);
  }, []);
  ...
  const tabelaOptions = useMemo(() => ({
    ...
    showExport: true,
    showFooter: true,
    fileName: "abertura-competencia",
    rowSelection: true,
    rowSelectionMode: "multiple",
    onRowSelectChange: handleGetDadosConfirmar,
    additionalButtons: [
      {
        title: 'Efetuar Abertura',
        icon: 'fa-solid fa-door-open',
        onClick: handleEfetuarAbertura
      }
    ]
  }), [handleGetDadosConfirmar, handleEfetuarAbertura]);

  return (
    <div className={styles.aberturaContainer} id="abertura-competencia-container">
      <h2 className={styles.aberturaTitle}>
        <i className="fa-solid fa-door-open"></i>
        Abertura por Competência
      </h2>
      
      <div className={styles.aberturaContent}>
        ...
        <ModalMotivoAbertura 
          isOpen={showModalMotivoAbertura}
          onClose={handleCloseModalAbertura}
        />
      </div>
    </div>
  );
